...
boost::asio::ip::tcp::resolver::query query(seed, std::to_string(CryptoNote::P2P_DEFAULT_PORT));
...
while (iter != end)
{
	...
	std::string st = boost::lexical_cast<std::string>(ep);
        append_net_address(m_seed_nodes,st);
        logger(INFO,BRIGHT_GREEN) << "Found IP: " << st;
}
