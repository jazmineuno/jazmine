...
NativeContext& Dispatcher::getReusableContext() {
  if(firstReusableContext == nullptr) {
   ...
   makecontext(static_cast<ucontext_t *>(newlyCreatedContext), reinterpret_cast<void(*)()>(contextProcedureStatic), reinterpret_cast<intptr_t>(&makingData));
   
   uctx* oldContext = static_cast<ucontext_t *>(currentContext->uctx);
